...
function toUiShape(conditions: LocalCondition[]) {
  ...
  // Map conditions to specific medicines and doctor types
  const conditionContext: { [key: string]: { medicines: string[], doctorType: string } } = {
    "Common Cold": { medicines: ["Paracetamol", "Cetirizine", "Cough Syrup"], doctorType: "General Physician" },
    "Bronchitis": { medicines: ["Guaifenesin", "Salbutamol"], doctorType: "Pulmonologist" },
    "Pneumonia": { medicines: ["Azithromycin", "Amoxicillin"], doctorType: "Pulmonologist" },
    "Asthma": { medicines: ["Salbutamol Inhaler", "Montelukast"], doctorType: "Pulmonologist" },
    "Gastritis": { medicines: ["Omeprazole", "Ranitidine"], doctorType: "Gastroenterologist" },
    "Food Poisoning": { medicines: ["ORS", "Domperidone"], doctorType: "General Physician" },
    "Migraine": { medicines: ["Ibuprofen", "Sumatriptan"], doctorType: "Neurologist" },
    "Arthritis": { medicines: ["Diclofenac", "Ibuprofen"], doctorType: "Orthopedic" },
    "Heart Attack": { medicines: ["Aspirin"], doctorType: "Cardiologist (Emergency)" },
  }
  ...
  return {
    conditions: conditions.map((c) => ({
      ...
      suggestedMedicines: conditionContext[c.name]?.medicines || ["Consult doctor for medicine"],
      suggestedDoctorType: conditionContext[c.name]?.doctorType || "General Physician"
    })),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check environment first, then fallback to request headers
