                      ...
                      {analysisResult.conditions?.map((condition: any, index: number) => (
                        <div key={index} className="border rounded-lg p-4">
                          ...
                          <div className="space-y-3 pt-2 border-t border-gray-100">
                            <div>
                              <p className="text-xs font-semibold text-gray-500 uppercase mb-2">Suggested Doctor</p>
                              <Link href={`/doctors?specialty=${encodeURIComponent(condition.suggestedDoctorType)}`}>
                                <Button variant="outline" size="sm" className="h-8 text-xs bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100">
                                  <Users className="h-3 w-3 mr-1" />
                                  Find {condition.suggestedDoctorType}
                                </Button>
                              </Link>
                            </div>
                            
                            <div>
                              <p className="text-xs font-semibold text-gray-500 uppercase mb-2">Relevant Medicines</p>
                              <div className="flex flex-wrap gap-2">
                                {condition.suggestedMedicines?.map((med: string, mIdx: number) => (
                                  <Link key={mIdx} href={`/medicines?q=${encodeURIComponent(med)}`}>
